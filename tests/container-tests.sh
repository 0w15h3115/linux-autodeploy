...
# ------------------------------------------------------------------------------
head_ "R8: kali-deploy-remote has no WSL special-casing"
# ------------------------------------------------------------------------------
# The inverse of what R8 used to assert. This script carried a WSL profile that
# skipped tailscale, ssh and firewall -- its entire access layer -- which meant
# WSL was running it with the whole reason it exists switched off. That target
# moved to kali-deploy-wsl, and this test now pins the split so the special
# case cannot drift back in one convenience commit at a time.
#
# /run/WSL is a plain directory created by the WSL2 init, so the strongest form
# of the check is available here: make the box look exactly like WSL to the
# detection that used to run, and assert nothing changes.

mkdir -p /run/WSL
as_sudo --dry-run >/tmp/rwsl.log 2>&1

grep -q 'Target:.*WSL' /tmp/rwsl.log \
    && fail "still reports a WSL target -- detection came back" \
    || pass "no WSL detection in kali-deploy-remote"

wsl_skipped=""
for p in tailscale ssh firewall; do
    grep -q "=== Phase: $p ===" /tmp/rwsl.log || wsl_skipped="$wsl_skipped $p"
done
[[ -z "$wsl_skipped" ]] \
    && pass "access layer runs even when /run/WSL exists" \
    || fail "access layer skipped on a WSL-looking box:$wsl_skipped"

# The web tooling went to kali-deploy-wsl with the profile. A teamserver reached
# over a tailnet is not the box you fuzz from.
"$REMOTE" --print-packages | grep -qxE 'ffuf|gobuster|sqlmap' \
    && fail "WSL_PKGS survived in --print-packages" \
    || pass "web tooling no longer in this script's package set"

grep -qi 'wsl' <("$REMOTE" --list-phases) \
    && fail "a phase name mentions WSL" \
    || pass "no WSL phase"
